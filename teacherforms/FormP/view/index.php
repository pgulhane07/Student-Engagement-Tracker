...
$_SESSION['flag']=0;
	if(($_SESSION['login_flag'] == 3) || ($_SESSION['login_flag'] == 2 &&  $_SESSION['Desg'] == 'Principal')){
		$sql = 'SELECT a.*,at.Full_Name FROM formp a INNER JOIN Authority at ON a.ID = at.ID';
		$print ='FormP:- All Teachers';
	}
	else if($_SESSION['login_flag'] == 2 && ($_SESSION['Desg'] == 'HOD' || isset($_SESSION['CH']))){
		$sql = "SELECT a.*,at.Full_Name FROM formp a INNER JOIN Authority at ON a.ID = at.ID WHERE at.Department ='$dept'";
		$print ='FormP:- '.$dept.' Department';
	}
