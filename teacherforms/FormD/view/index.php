...
$_SESSION['flag']=0;
	if(($_SESSION['login_flag'] == 3) || ($_SESSION['login_flag'] == 2 &&  $_SESSION['Desg'] == 'Principal')){
		$sql = 'SELECT d.*,at.Full_Name FROM formd d INNER JOIN Authority at ON d.ID = at.ID';
		$print ='FormD:- All Teachers';
	}
	else if($_SESSION['login_flag'] == 2 && ($_SESSION['Desg'] == 'HOD' || isset($_SESSION['CH']))){
		$sql = "SELECT d.*,at.Full_Name FROM formd d INNER JOIN Authority at ON d.ID = at.ID WHERE at.Department ='$dept'";
		$print ='FormD:- '.$dept.' Department';
	}
