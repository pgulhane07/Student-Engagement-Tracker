...
$sql='';
  ...
  $_SESSION['order'] = $sort_order;
    ...
    if($flag == 0){
      if(($_SESSION['login_flag'] == 3) || ($_SESSION['login_flag'] == 2 &&  $_SESSION['Desg'] == 'Principal')){
        ...
        $sql="SELECT s.*,st.Rollno,st.Year,st.Full_Name FROM social_work s INNER JOIN Student st ON s.ID = st.ID WHERE st.Department ='$dept' ORDER BY  ".$column." ".$sort_order;
      }
  ...
  elseif($flag == 1){
    if(($_SESSION['login_flag'] == 3) || ($_SESSION['login_flag'] == 2 &&  $_SESSION['Desg'] == 'Principal')){
      ...
      $sql = "SELECT s.*,st.Rollno,st.Year,st.Full_Name FROM social_work s INNER JOIN Student st ON s.ID = st.ID where st.Department ='$dept' AND $sql_search ORDER BY ".$column." ".$sort_order;
    }
    //echo $sql;
    $statement = $connection->prepare($sql);
    if ($statement->execute()) {
  ...
  elseif($flag == 2){
    if(($_SESSION['login_flag'] == 3) || ($_SESSION['login_flag'] == 2 &&  $_SESSION['Desg'] == 'Principal')){
      ...
      $sql = "SELECT s.*,st.Rollno,st.Year,st.Full_Name FROM social_work s INNER JOIN Student st ON s.ID = st.ID where st.Department ='$dept' AND $sql_dept ORDER BY ".$column." ".$sort_order;
    }
    //echo $sql;
    $statement = $connection->prepare($sql);
    if ($statement->execute()) {
  ...
  elseif($flag == 21 || $flag == 12){
    if(($_SESSION['login_flag'] == 3) || ($_SESSION['login_flag'] == 2 &&  $_SESSION['Desg'] == 'Principal')){
      ...
      $sql = "SELECT s.*,st.Rollno,st.Year,st.Full_Name FROM social_work s INNER JOIN Student st ON s.ID = st.ID where st.Department ='$dept' AND $sql_search AND $sql_dept ORDER BY ".$column." ".$sort_order;
    }
